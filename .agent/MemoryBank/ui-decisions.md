...
### 週間予報（ForecastStrip）
- 横スクロール可能なカードリストとして実装（`snap-x` でスナップ）。各カードに日付、アイコン、降水確率、最高/最低気温を表示。

### レイアウト
- ウィンドウサイズ: 未決（`projectbrief.md` の未決事項参照）
...
- 実装は v2 以降（v1はアニメーションなし・静的SVGのみ）
- v2実装時の仕様:
  - 晴れ: 太陽の光芒が緩やかに回転（20秒/周）
  - くもり: 雲が左右にゆっくり流れる（30秒/往復）
  - 雨: 雨粒が上から下に流れる（1.5秒/周）
  - 雪: 雪片がゆっくり落下（3秒/周）
