    ...
    {#if $weatherStore.warning.severity !== 'none'}
      <div class="alert {$weatherStore.warning.severity === 'warning' ? 'alert-error' : 'alert-warning'} shadow-sm">
        ...
        <div>
          ...
          {#if $weatherStore.warning.headline}
            <p class="text-xs">{$weatherStore.warning.headline}</p>
          {/if}
          <p class="text-xs">{$weatherStore.warning.kinds.join('・')}</p>
        </div>
      </div>
    {:else if $weatherStore.warning.error}
      <div class="alert alert-warning shadow-sm">
        ...
        <div>
          <p class="font-semibold text-sm">警報情報を確認できませんでした</p>
          <p class="text-xs">{$weatherStore.warning.error}</p>
        </div>
      </div>
    {/if}
